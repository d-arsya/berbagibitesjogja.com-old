...
@section('container')
    ...
    <form method="POST" action="{{ route('volunteer.update', $volunteer->id) }}"
        ...
        <div class="relative z-0 w-full mb-5 group">
            <input type="text" name="name" id="name" value="{{ $volunteer->name }}"
        ...
        <div class="relative z-0 w-full mb-5 group">
            ...
            <label for="password"
                class="peer-focus:font-medium absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-blue-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">Password</label>
        </div>
        @if (auth()->user()->id == $volunteer->id || auth()->user()->role == 'super')
            <div class="w-full py-9 bg-gray-50 rounded-2xl border border-gray-300 gap-3 grid border-dashed">
                <div class="grid gap-1">
                    ...
                    <h2 class="text-center text-gray-400   text-xs leading-4">PNG or JPG, smaller than 5MB</h2>
                </div>
